...
chrome.storage.local.get(['preferredModel', 'useVllm'], (res) => {
...
function renderSimilarArticles(articles) {
    ...
    articles.forEach(article => {
        ...
        const faviconUrl = `https://www.google.com/s2/favicons?domain=${sourceDomain}&sz=64`;
        const summaryHtml = article.summary ? `<div class="similar-summary">${article.summary}</div>` : '';

        card.innerHTML = `
            <div class="favicon-container">
                <img src="${faviconUrl}" class="favicon-img" onerror="this.src='icons/icon16.png'">
            </div>
            <div class="similar-info">
                ...
                <div class="similar-meta">
                    ...
                    <a href="${article.url}" target="_blank" class="link-badge">Link</a>
                </div>
            </div>
        `;
        ...
        const resultLow = (data.result || '').toLowerCase();
            ...
            if (sources.length > 0) {
                const badgesHTML = sources.map((src, idx) => 
                    `<a href="${src.url}" target="_blank" rel="noopener noreferrer" class="source-badge">🔗 ${src.title || 'Source ' + (idx + 1)}</a>`
                ).join('');
                sourcesBadgesHTML = `
